...
export default function ChatModal({ showChatModal } : { showChatModal: any }) {
  ...
  const { data: messages, mutate: mutateMessages, isLoading: isLoadingMessages } = useStaleSWR(showChatModal ? `/api/issues/${showChatModal.id}/messages` : null);
  ...
  const Message = ({ message }: { message: any }) => {
    if (parseInt(user?.id) === message.userId) {
      return (
        ...
        <div
          className="py-3 px-4 bg-green-400 rounded-bl-xl rounded-tl-xl rounded-br-xl text-white"
        >{message.message}</div>
        <Image
    ...
    return (
      ...
      />
      <div
        className="py-3 px-4 bg-gray-400 rounded-br-xl rounded-tr-xl rounded-bl-xl text-white"
      >{message.message}</div>
    </div>
    );
  };
